...
const Utilities = {
  // sortShows: (items: ItineraryItems[]) => {
  ...
  // sortCashFlowReasons: (reasons: string[]) => {
  ...
  // sentenceCaseWords: (words: string) => {
  //   // debugger
  ...
  //   }
  // },
  formatTimeStamp: (date: string, strFormat: string) => {
    var dateStr = format(new Date(parseInt(date) * 1000), strFormat)
    return dateStr
  },
}
